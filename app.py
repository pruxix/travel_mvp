...
QUESTIONS = [
    {"id": "purpose",       "text": "Какова главная цель поездки?",           "type": "radio",  "emoji": "🎯",
     "options": ["Отдых и релаксация", "Приключения и активный туризм", "Культура и история", "Гастрономия и шопинг", "Работа"]},
...
{{"traveler_type":"тип (1-3 слова)","archetype":"архетип","motivation":"мотивация (1 предложение)","style":"стиль (1-2 предложения)","strengths":["сила 1","сила 2","сила 3"],"challenges":["сложность"],"perfect_destination_type":"идеальный тип (максимум 2 слова)"}}"""
...
КРИТИЧЕСКИ ВАЖНО: пиши маршрут ПОЛНОСТЬЮ, не останавливайся на середине, доведи до конца все разделы. Цены пиши в рублях. При этом строго учитывай выбранный бюджет. Учти реалистичные цены на билеты/отель и так далее, также учитоывай количество людей и указывай, на какое количество сделать расчет.
Добавляй цены на музеи, если они есть, и выгодные условия. Также добавь короткеи заметки о том, что в том или ином месте может быть многолюдно (например, вечером в популярном ресторане). 
При общении с клиентом учитывай все его правки в маршрут, и при этом учитывай временные отрезки.
